        ...
        <>
          <div className="min-h-screen bg-white flex flex-col items-center px-[10%] py-8">
            ...
            <div className="relative flex items-center justify-between w-full mb-10 mt-10">
              {/* Connecting Lines */}
              <div className={`absolute top-5 left-10 right-1/2 h-1 z-0 mr-4 ${currentStep >= 2 ? 'bg-red-700' : 'bg-gray-300'
                }`}></div>
              <div className={`absolute top-5 left-1/2 right-10 h-1 z-0 ml-5 ${currentStep >= 3 ? 'bg-red-700' : 'bg-gray-300'
                }`}></div>

              {/* Step 1 */}
              <div className="flex flex-col items-center z-10">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold ${currentStep >= 1
                    ? 'bg-red-700 text-white'
                    : 'border-2 border-gray-400 bg-white text-gray-500'
                  }`}>
                  {currentStep >= 1 ? '✓' : '1'}
                </div>
                <span className={`mt-2 font-bold text-sm ${currentStep >= 1 ? 'text-black' : 'text-gray-400'
                  }`}>SUBMITTED</span>
              </div>

              {/* Step 2 */}
              <div className="flex flex-col items-center z-10">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold ${currentStep >= 2
                    ? 'bg-red-700 text-white'
                    : 'border-2 border-gray-400 bg-white text-gray-500'
                  }`}>
                  {currentStep >= 2 ? '2' : '2'}
                </div>
                <span className={`mt-2 font-bold text-sm ${currentStep >= 2 ? 'text-black' : 'text-gray-400'
                  }`}>IN PROGRESS</span>
              </div>

              {/* Step 3 */}
              <div className="flex flex-col items-center z-10">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold ${currentStep >= 3
                    ? 'bg-red-700 text-white'
                    : 'border-2 border-gray-400 bg-white text-gray-500'
                  }`}>
                  {currentStep >= 3 ? '✓' : '3'}
                </div>
                <span className={`mt-2 font-bold text-sm ${currentStep >= 3 ? 'text-black' : 'text-gray-400'
                  }`}>APPROVED</span>
              </div>
            </div>
            {/* Proposal Details */}
