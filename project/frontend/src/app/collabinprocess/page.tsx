...
export default function CollabInProcess() {
    ...
    useEffect(() => {
        const fetchProposals = async () => {
            ...
            try {
                ...
                // filter only proposals with "ONGOING" status
                const ongoing = (Array.isArray(data) ? data : data.proposals || []).filter(
                    (p: any) => p.status?.toUpperCase() === "ONGOING"
                );

                setProposals(ongoing);
            } catch (err) {
